...
import { SearchPageProps, SearchPageState } from '../../interfaces/stateProps';
      ...
      if (userSearch === '' || userSearch === undefined) {
        ...
        BooksAPI.search(this.state.userSearch, 30)
          .then(initialSearchResults => {
            const updatedSearchResults = initialSearchResults.map(
              (result: BookObject) => {
        ...
        <Styles.SearchBooksResults>
          <Styles.BooksGridOL>
            ...
            {this.state.updatedSearchResults.length > 0 &&
              this.state.updatedSearchResults.map(
                (searchResult: BookObject) => (
                  <Book
                    key={searchResult.id}
                    book={searchResult}
                    bookImgUrl={
